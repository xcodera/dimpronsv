...
import {
    ...
    LogOut, ShieldCheck, Calendar, Wallet, Star, TrendingUp, CheckCircle2,
    Users, Search, Landmark, Megaphone
} from 'lucide-react';
import { MOCK_ACCOUNT, UserProfile } from '../constants';
...
export const Dashboard: React.FC<DashboardProps> = ({ isDarkMode, toggleDarkMode, setActiveView, activities, currentUser }) => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    try {
                        // FIX: Add User-Agent header for robust geocoding requests.
    ...
    const MOTIVATIONS = [
        ...
        "Fokus pada tujuanmu, bukan hambatan.",
        "Kerja keras hari ini adalah kemenangan esok hari.",
        "Percayalah pada proses dan nikmati perjalanannya.",
    ];

    const motivationText = useMemo(() => {
        return [greeting, ...MOTIVATIONS].join(' ++ ');
    }, [greeting]);
    ...
    {/* Header Card */ }
    < div className = {`px-6 pt-12 pb-6 relative overflow-hidden transition-colors duration-500 flex justify-between items-center rounded-b-[2.5rem] ${isDarkMode ? 'bg-gradient-to-r from-blue-900/40 to-indigo-900/40 text-white' : 'bg-gradient-to-r from-[#004691] to-[#00AEEF] text-white'
        }`}>
                    ...
                    <div className={ `w-14 h-14 rounded-full border-2 border-white/20 overflow-hidden shadow-md bg-gray-500` }>
                        <img src={ currentUser.avatarUrl } alt = "Profile" className = "w-full h-full object-cover" />
                            ...
                            <p className="text-sm font-light text-blue-200" > Welcome Back, </p>
                                < h1 className = "text-white font-black text-xl leading-tight tracking-tight" > { currentUser.name } </h1>
                                    < p className = "text-xs text-blue-200/80 mt-0.5" > { currentUser.jobTitle } </p>
                                        </div>
                                        </div>
                        ...
                        < div className = "px-6 pt-8 pb-8 space-y-8" >
                            ...
                            < div className = {`p-4 rounded-3xl shadow-lg transition-colors ${isDarkMode ? 'bg-[#1e293b] text-white border border-white/10' : 'bg-gray-100 text-gray-800'}`}>
                                <div className="flex items-baseline mb-3 overflow-hidden whitespace-nowrap" >
                                    <h2 className={ `font-bold text-lg shrink-0 ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}` }>
                                        Hello, { userFirstName }
                                        </h2>
                                        < div className = "relative flex overflow-x-hidden text-sm text-gray-500 ml-2 w-full" >
                                            <div className="animate-marquee" >
                                                <span className="mx-4" > { motivationText } </span>
                                                    </div>
...
label = "Laporan"
    ...
    <MenuIcon 
            isDarkMode={ isDarkMode }
onClick = {() => setActiveView('ads-reports')}
icon = {< Megaphone size = { 24} className = "text-cyan-500" />}
label = "Reports Ads"
    />
    </div>

{/* Ringkasan Akun & Kinerja */ }
<div className="space-y-4" >
    <div className="flex justify-between items-center px-1" >
        <h3 className={ `font-bold ${isDarkMode ? 'text-gray-200' : 'text-[#004691]'}` }> Ringkasan Akun & Kinerja </h3>
            </div>
...
label = "Total Leads"
value = { currentUser.stats.totalLeads }
detail = "Update 5m ago"
    />
    <StatCard
              isDarkMode={ isDarkMode }
icon = {< Search size = { 20} className = "text-yellow-500" />}
label = "Total Sliks"
value = { currentUser.stats.totalSliks }
detail = "Bulan Ini"
    />
    <StatCard
              isDarkMode={ isDarkMode }
...
value = { currentUser.stats.peningkatanPenjualan }
detail = "+12% target"
    />
    <StatCard
              isDarkMode={ isDarkMode }
...
label = "Total BAST"
value = { currentUser.stats.totalBAST }
...
className = {`text-xs font-black uppercase tracking-widest ${isDarkMode ? 'text-blue-400' : 'text-[#004691]'}`}
            >
    Lihat Semua
        </button>
        </div>
...
const ActivityItem: React.FC<{ activity: Activity; isDarkMode: boolean }> = ({ activity, isDarkMode }) => {
    const getIcon = () => {
...
return (
    ...
    <div className="flex items-center gap-3" >
        <div className={ `w-10 h-10 rounded-2xl flex items-center justify-center transition-colors shrink-0 ${bg}` }>
            { icon }
            </div>
            < div className = "flex flex-col" >
                <p className={ `font-bold text-sm leading-none ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}` }>
                    { activity.title }
                    </p>
                    < p className = "text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-0.5 leading-none" >
                        ...
                        < div className = "text-right shrink-0" >
                            <p className={ `font-black text-sm leading-none ${isDarkMode ? 'text-gray-200' : 'text-[#004691]'}` }>
...
const MenuIcon: React.FC<{ icon: React.ReactNode; label: string; isDarkMode: boolean; onClick?: () => void }> = ({ icon, label, isDarkMode, onClick }) => (
    <button onClick= { onClick } className = "flex flex-col items-center gap-2 group" >
        <div className={`w-14 h-14 rounded-2xl shadow-sm flex items-center justify-center group-active:scale-90 transition-all border ${isDarkMode
                ? 'bg-white/5 border-white/10 group-hover:bg-white/10'
                : 'bg-white border-gray-100 shadow-blue-900/5 group-hover:shadow-md'
            }`}>
                { icon }
                </div>
...
const StatCard: React.FC<{
    ...
    detail: string;
    isDarkMode: boolean;
}> = ({ icon, label, value, detail, isDarkMode }) => (
